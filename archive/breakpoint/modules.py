...
#convert obs_lon to same grid as model 
    if obs_lon > lon_e[-1]:
...
#find gridbox number of model for obs data
    if lat_i > 0:
        lat_i = (len(lon_e)-1)*lat_i

    gridbox_num = lat_i + lon_i
...
def take_lomb(time,var,OFAC,SAMP_R):
    ...
    NOUT = 0.5*int(OFAC)*HIFAC*len(time)
    NOUT = int(NOUT)
		
	#take lomb
    fa, fb, mag, ph, s_val_ind, end_val_ind, fr, fi = lomb_phase.lomb(time,var,NOUT,OFAC,SAMP_R)    
    amp_corr = 1./(sum(window)/len(window))
    corr_mag = mag * amp_corr
    
	#limit arrays by valid indices determine in fortran code
    fa = fa[s_val_ind:end_val_ind]
    fb = fb[s_val_ind:end_val_ind]
    corr_mag = corr_mag[s_val_ind:end_val_ind]
    ph = ph[s_val_ind:end_val_ind]
    fr = fr[s_val_ind:end_val_ind]
    fi = fi[s_val_ind:end_val_ind]
    
    periods = 1./fa
    	
    return periods,corr_mag,ph,fr,fi

def take_fft(time,var):
    window = signal.hanning(len(var))
    var_mean = np.mean(var)
    var = var - var_mean
    var = var*window
    ...
    fft_freqs = fftfreq(len(var),SAMP_R)
    valid = fft_freqs > 0
    fft_periods = 1./fft_freqs
    fft_mag = fft_mag/len(time)
    fft_periods,fft_mag = fft_periods[valid],fft_mag[valid]
    fft_mag = fft_mag*2
    amp_corr = 1./(sum(window)/len(window))
    fft_mag = fft_mag * amp_corr

    return fft_periods,fft_mag
...
def get_tags(obs_refs):
	...
	tag_dict = {'arh':'ANT','ask':'AF','bhd':'OC','bmw':'O','brt':'NA','brw':'NA','cgo':'OC','cmn':'EU','cpt':'AF','cvo':'O','dcc':'ANT','deu':'EU','dig':'EU','glh':'EU','hpb':'EU','irb':'EU','izo':'OC','jfj':'EU', \
				'kmw':'EU','kos':'EU','kps':'EU','kvk':'EU','kvv':'EU','lgb':'EU','mlo':'O','mnm':'O','ngl':'EU','nmy':'ANT','nwr':'NA','pal':'EU','pay':'EU','pyr':'AS','rcv':'EU', \
...
def find_breakpoint(periods,mag):
    #log bin smooth
    periods, mag = log_bin_median(len(periods),30,periods,mag)
    ...
    periods = np.log(periods)
    mag = np.log(mag)
    periods+=30
    mag+=30
		
    #find max gradient change
    array_s = 1
    start_i = 0
    end_i = array_s
    max_grad_diff = 0
    ...
    for i in range(len(periods)-1):
        rise_x = periods[end_i] - periods[start_i]
        rise_y = mag[end_i] - mag[start_i]
		    
        current_grad = rise_y/rise_x
        try:
            current_grad_diff = last_grad-current_grad
            if current_grad_diff > max_grad_diff:
                #limit break point range between 1.5 and 15
                if (periods[i] > (np.log(1.5))+30) & (periods[i] < (np.log(15))+30):
                    max_grad_diff = current_grad_diff 
                    max_grad_index = i
                else:
                    max_grid_diff = 0
            last_grad_diff = current_grad_diff
            last_grad = grad
        except:
    ...
    breakpoint = periods[max_grad_index]

    periods = periods-30
    mag = mag-30
    breakpoint = breakpoint-30
    periods = np.exp(periods)
    mag = np.exp(mag)
    breakpoint  = np.exp(breakpoint)
		
    return periods,mag,breakpoint
