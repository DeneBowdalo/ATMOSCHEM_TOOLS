...
# Readin the model output
        ...
        model , names = readfile("GEOS_v90102_4x5_CV_logs.npy","CVO") #001 represents CVO
# Processes the date 
        year=(model[:,0]//10000)
...
#Need to normalise model data also
    if model_cut_switch == 0:
        ...
        print np.min(model_cut)
        print np.max(model_cut)  	
...
#Plot up standard conc. v time plot
    ...
    plt.plot(since2006, model_cut, color='green', label='GEOS v9.01.02 4x5 ')
    plt.grid(True)
...
#Divide power by respective frequencies to get change power units
    fb = [a*b for a, b in zip(fa,fb)]
    fb = [np.sqrt(a) for a in fb]
...
#Divide power by respective frequencies to get change power units
    fy = [a*b for a, b in zip(fx,fy)]
    fy = [np.sqrt(a) for a in fy]
    print np.max(fy)
#plot up
    plt.loglog(1./fx, fy, 'gx', alpha = 0.75,markersize=2, label='GEOS v9.01.02 4x5 ')
