...
# Readin the model output
        ...
        model , names = readfile(glob.glob("logs/plane*"),"001") #001 represents CVO
# Processes the date 
        year=(model[:,0]//10000)
        ...
        first= 0
        last = 24
        length =  len(since2006)/24
        print length
        hourly = []
        hourly_species = []
        for i in range(length):
            averaged_time = since2006[first]+1 
            averaged_species = np.average(model_cut[first:last]) 
            hourly.append(averaged_time)
            hourly_species.append(averaged_species)
        ...
        #hourly_species=np.array(hourly_species)
        #hourly_species=hourly_species.flatten()
        #print hourly.shape
        #print hourly_species.shape 
        for i in hourly:
            stringed = str(i)
            new_hourly.append(stringed)

        for i in hourly_species:
            stringed = str(i)
            new_hourly_species.append(stringed)  
        
        #joined = np.hstack((hourly,hourly_species))
        #joined =  joined.astype(str)         
        #print joined
        #print joined.shape
        
        #joined = np.reshape(joined,(length,-1))
        #print joined
        text_file = open("O3_daily_CV.txt", "w")
        #print joined.shape 
        for i,j in zip(new_hourly,new_hourly_species):
            text_file.write(i+' '+j+'\n')
