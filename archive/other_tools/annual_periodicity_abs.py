...
#calculate abs. diff
	...
	plt.plot(period_array,compare_powers, color=cm.jet(1.*i/len(mversion)) , marker='x', alpha = 0.75, markersize=2, label = 'GEOS-Chem %s %s %s %s PSD - %s Obs. PSD' %(mversion[counter], res[counter], met[counter],actual_species_name, actual_species_name))
	#plt.xlim(200,500)
	#plt.ylim(-500,800)
	plt.grid(True)
	...
	plt.ylabel(r'Absolute Diff. in PSD $(ppb^{2}/days^{-1})$', fontsize=21)
	plt.title('Annual Model PSD - Obs. PSD at %s' %(location),fontsize=21)
	counter+=1
