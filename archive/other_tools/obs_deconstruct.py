...
#read in obs data
root_grp = Dataset('../O3/GAW_O3/GAW_SURFACE_O3_2006_2012.nc')

#read in specific site data
site_group = root_grp.groups['MHD']
...
#remove invalid data
periods, mag, ph,fr,fi,amp_corr= modules.take_lomb(obs_times_from0,obs_var,4,1./24)
...
ha_wave = ha_amp*(np.cos((pi2*obs_times_full/(365.25/2.))-(ha_phase)))
annual_wave = annual_amp*(np.cos((pi2*obs_times_full/365.25)-(annual_phase)))
...
ax.plot(full_obs_datetimes,big_wave,alpha=0.5,label='Diurnal + Half-Annual + Annual',color='yellow')
ax.plot(full_obs_datetimes,daily_wave,alpha = 0.6,linestyle = ':',color='green',linewidth=8,label='Diurnal')
ax.plot(full_obs_datetimes,ha_wave,linestyle = '-.',color='blue',linewidth=8,label='Half-Annual')
ax.plot(full_obs_datetimes,annual_wave,linestyle = '--',color='red',linewidth=8,label='Annual')
