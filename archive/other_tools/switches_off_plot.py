...
if GAW_switch == 'y':
	model , names = modules.readfile_GAW("binary_logs/convectoff_gaw_logs_O3.npy",model_index) #model index represents gaw location
else:
...
#Smooth using exponetial smoother, last 2 numbers inputted are: smoothing window & cut-off point
	cut_obs_periods, smoothed_obs = modules.ema(reversed_fb,reversed_obs_periods,10,120)
...
#print nyquist_freq_lomb_model, Si_lomb_model, Si_lomb_model*2 
	...
	#plt.savefig('plots/v90103_nestedeurope_1year_GEOS5/smoothed/%s_%s_%s_%s.png'%(actual_species_name,location,mversion,res),dpi=fig.dpi)
