...
for f in files:
	...
	#LSP
	fa, fb, fft_model, fft_phase= lomb_phase.lomb(times,read)
	model_periods = 1./fa
	...
	#save fft magitude
	np.save('chunked_mags_phases/LSP/%s_%s/MH_obs_daily_magnitudes'%(start_year,end_year),daily_mag_array)
	np.save('chunked_mags_phases/LSP/%s_%s/MH_obs_half_annual_magnitudes'%(start_year,end_year),half_annual_mag_array)
	np.save('chunked_mags_phases/LSP/%s_%s/MH_obs_annual_magnitudes'%(start_year,end_year),annual_mag_array)

	#save fft phase
	np.save('chunked_mags_phases/LSP/%s_%s/MH_obs_daily_phases'%(start_year,end_year),daily_phase_array)	
	np.save('chunked_mags_phases/LSP/%s_%s/MH_obs_half_annual_phases'%(start_year,end_year),half_annual_phase_array)
	np.save('chunked_mags_phases/LSP/%s_%s/MH_obs_annual_phases'%(start_year,end_year),annual_phase_array)
