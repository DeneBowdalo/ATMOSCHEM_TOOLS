...
#read obs files
all_files = glob.glob('/work/home/db876/observations/surface/%s/CAPMON/*'%(species))
...
for i in range(len(all_files)):
    f = all_files[i]
    f = f.replace("/work/home/db876/observations/surface/%s/CAPMON/"%(species), "")
    f = f[11:14]
    ref_list.append(f)
...
#setup netcdf output
root_grp = Dataset('CAPMON_SURFACE_%s_%s_%s_%s.nc'%(fname_species,start_year,end_year+1,output_res), 'w')
root_grp.description = 'Hourly Surface %s at CAPMON sites - Program written by Dene Bowdalo'%(fname_species)
...
#for ref_i in range(len(valid_refs)):
    ...
    converted_time,vals,mm_big,st_big,na = modules.remove_duplicate_points(site_ref,converted_time,vals,mm_big,st_big,'blank',output_res)
    test = vals >= 0
    ...
    country = 'Canada'
    contact = 'Dave MacTavish, 4905 Dufferin St., Toronto ON, CANADA, M3H 5T4, Dave.MacTavish@ec.gc.ca'
...
if run_type == 'serial':
    for c in range(len(valid_refs)):
        ...
        #change ref name
        valid_refs[c] = 'capmon'+valid_refs[c]
        #add counts up
