...
dt_dates = [d.strftime('%Y%m%d') for d in pd.date_range(start,end,freq='D')][:-1]
dt_times = [d.strftime('%H%M') for d in pd.date_range(start,end,freq='D')][:-1]
...
key = lambda x: pd.Period(str(x.year)+'-'+str(x.month)+'-'+str(x.day))

full_data = np.empty((n_days,n_lats,n_lons))
...
for species in species_list:
    ...
    root_grp = Dataset('GEOSCHEM_SURFACE_%s_%s_%s_%s_%s_GEOS5_D_*.nc'%(species,start_year,end_year+1,model_vers,grid_res), 'w')
    root_grp.description = 'Gridded DAILY Surface %s in ppb - Program written by Dene Bowdalo'%(species)

    # dimensions
    #set variables
    root_grp.createDimension('%s'%(species.lower()), n_days)
    root_grp.createDimension('dates', n_days)
    root_grp.createDimension('times', n_days)
