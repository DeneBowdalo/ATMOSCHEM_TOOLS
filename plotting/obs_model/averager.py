...
#read in obs data
root_grp = Dataset('../process/GLOBAL_%s_%s_%s_%s_%s_PERIODIC.nc'%(vres,species,start_year,end_year,timeres))
...
period = raw_input('\nChoose Average Type.\nDAY,MONTH,ANN or profiles: HOUR_P.DAY_P,MONTH_P,ANN_P\n')

if period in ['MONTHLY', 'MONTH','M','MON']:
    obs_key = lambda x: pd.Period(str(x.year)+'-'+str(x.month))
    model_key = obs_key
elif period in ['ANN', 'ANNUAL', 'ANNUALLY', 'A', 'YEARLY', 'YEAR', 'YR', 'Y']:
    obs_key = lambda x: pd.Period(str(x.year))
    model_key = obs_key
elif period in ['DAILY', 'DAY', 'D']:
    obs_key = lambda x: pd.Period(str(x.year)+'-'+str(x.month)+'-'+str(x.day))
    model_key = obs_key
elif period in ['HOURLY','HOUR','H']:
    obs_key = lambda x: pd.Period(str(x.hour))    
    model_key = obs_key
elif period == 'HOUR_P':
    obs_key = [obs_time_pd.hour]
    model_key = [model_time_pd.hour]
elif period == 'DAY_P':
    obs_key = [obs_time_pd.day]
    model_key = [model_time_pd.day]   
elif period == 'MONTH_P':
    obs_key = [obs_time_pd.month]
    model_key = [model_time_pd.month]
elif period == 'ANN_P':
    obs_key = [obs_time_pd.year]
    model_key = [model_time_pd.year]
#elif period == 'MON_DESEAS':
#    key = lambda x: (x.year, x.month)
#    key1 = lambda x: x.month
...
obs_mean[datafrac<0.5]=np.NaN

#group the data
group=model_var_pd.groupby(model_key)
#calculate the fraction of hours in the month with good data
datafrac=group.count().div(group.size(),axis='rows')
# calculate the monthly mean
model_mean = group.mean()
# now, zero out the mean data where datafrac<0.75
model_mean[datafrac<0.75]=np.NaN
...
#ax.plot(goodO3mean, color='black', markersize = 3, label = 'Observations')
obs_mean.plot(label='Observations', marker = 'x', markersize = 20, color = 'black')
model_mean.plot(label=model_chose, marker = 'x', markersize = 20, color = 'red')
#ax.plot_date(model_time_pd.to_pydatetime(), model_var_pd, color='red', markersize = 3, label = '%s %s %s %s'%(model_name,version,grid_size,met))
...
#ax.set_title(r'Time Series of Surface %s at %s, for Observations & %s %s \n %s'%(species,site_ref,model_name,model_range,note),fontsize=18)
#ax.set_title('Time Series of Surface %s at %s, for Observations & %s %s \n %s'%(species,site_ref,model_name,model_range,note),fontsize=18)
