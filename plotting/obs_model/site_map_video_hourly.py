...
#read in obs data
root_grp = Dataset('../process/GLOBAL_SURFACE_%s_%s_%s_H_PERIODIC.nc'%(species,start_year,end_year))
...
def animator(i):
    ...
    ax.set_title('HOURLY %s %s %s'%(species,full_dates[i],full_times[i]),fontsize=26)
    
print nframes

ani = animation.FuncAnimation(fig, animator, frames=nframes)

ani.save('%s_HOURLY_video.mp4'%(species), 'ffmpeg',fps=framerate,extra_args=['-vcodec','libx264', '-s','hd1080', '-pix_fmt', 'yuv420p'])
