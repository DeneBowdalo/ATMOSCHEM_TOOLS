...
obs_ts_grp = Dataset('/work/home/db876/observations/surface/%s/process/'%(species))
...
param = raw_input('\nmag, ph_min, ph_max, ave, ff or pv?\n')

if (param != 'ave') & (param != 'pv'):
...
if param == 'mag':
    if period_type == 'd':
        diff = obs_daily_mag
    if period_type == 's':
        diff = obs_seasonal_mag

if param == 'ph_max':
    if period_type == 'd':
        diff = obs_daily_phase
    if period_type == 's':
        diff = obs_seasonal_phase

if param == 'ph_min':
    if period_type == 'd':
        diff = obs_daily_phase_min
    if period_type == 's':
        diff = obs_seasonal_phase_min
...
if param == 'ff':
    if period_type == 'd':
        diff = obs_daily_ff
    if period_type == 's'
        diff = obs_seasonal_ff
        
if param == 'pv':
    if period_type == 'd':
        diff = diurnal_obs_frac
    if period_type == 's':
        diff = seasonal_obs_frac
    if period_type == 'a':
        diff = all_obs_frac
...
for ax in axes.flat:
    ...
    for i in range(len(current_lons)):
        ...
        if (param == 'ph_min') or (param == 'ph_max'):
            if (period_type == 'd'):
                all = m.scatter(X[i],Y[i],c=current_diff[i], s=m_size, vmin = 0,vmax = 24, marker = 'o',edgecolor='black',linewidth=0.5,cmap=plt.cm.hsv,zorder=10)    
            if (period_type == 's'):
                all = m.scatter(X[i],Y[i],c=current_diff[i], s=m_size, vmin = 0,vmax = 12, marker = 'o',edgecolor='black',linewidth=0.5,cmap=plt.cm.hsv,zorder=10)
        if param == 'ave':
            min_diff = np.min(abs(diff))
            max_diff = np.max(abs(diff))
            all = m.scatter(X[i],Y[i],c=current_diff[i], s=m_size, vmin = min_diff,vmax = max_diff, marker = 'o',edgecolor='black',linewidth=0.5,cmap=plt.cm.coolwarm,zorder=10)
        ...
        if param == 'pv':
            all = m.scatter(X[i],Y[i],c=current_diff[i], s=m_size, vmin = 0,vmax = 100, marker = 'o',edgecolor='black',linewidth=0.5,cmap=plt.cm.rainbow,zorder=10)
