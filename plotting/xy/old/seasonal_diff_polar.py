...
amplitude_diff = model_seasonal_mag - obs_seasonal_mag
phase_diff = model_seasonal_phase - obs_seasonal_phase
for d in range(len(phase_diff)):
    if phase_diff[d] > 6:
        phase_diff[d] = -6.+(phase_diff[d]-6.)
    elif phase_diff[d] < -6:
        phase_diff[d] = 6.-(np.abs(phase_diff[d])-6.)
...
ax1.set_xticks(ratio*np.arange(12))
ax1.set_xticklabels(['0','+1','+2','+3','+4','+5','+-6','-5','-4','-3','-2','-1'])
ax1.set_rgrids([5], angle=210.)
ax1.set_yticks([-15,-10,-5,0,5,10,15,20])
ax1.set_yticklabels(['-15','-10','-5','0','+5','+10','+15','+20'])
...
#plt.savefig('polar_seasonal_diff.png')
plt.show()
