...
def onpick(event):
    ...
    ts_i[ts_i < 0] = np.NaN
    p2.set_ydata(ts_i)
    ax2.set_ylim(np.nanmin(ts_i)-1,np.nanmax(ts_i)+1)
    ...
    p3.set_ydata(d_i)
    ax3.set_ylim(np.nanmin(d_i)-1,np.nanmax(d_i)+1)

    #seasonal_waveform
    s_i = obs_seasonal_waveform[ind,:]
    p4.set_ydata(s_i)
    ax4.set_ylim(np.nanmin(s_i)-1,np.nanmax(s_i)+1)
...
#ts plot
p2, = ax2.plot(obs_datetime_time,[0]*len(obs_datetime_time))
#ax2.set_xlim(0,50000)
             
#diurnal periodic plot
p3, = ax3.plot(obs_datetime_time[:24],[0]*24)
#ax3.set_xlim(0,24)
             
#seasonal periodic plot
p4, = ax4.plot(obs_datetime_time[:8766],[0]*8766)
#ax4.set_xlim(0,8766)
