...
end_year = int(present_dir[-3][-4:])
start_year_file = int(present_dir[-2][:4])
...
def run_LSP(vals,site_lon,x):
    ...
    if len(vals) == 0:
        ...
        #data should not have gaps > 1 year or persistent gap > 3 months
...
for i in range(len(obs_refs)):
    ref = obs_refs[i]
    ref_grp = root_grp.groups[ref]
    pressure_centres[i,:] = ref_grp.variables['pressure_centres'][:]

#read in model data
model_file = '/work/home/db876/plotting_tools/model_files/GEOSCHEM_VERT_%s_%s_%s_v902_2x2.5_GEOS5_D_*.nc'%(species,start_year_file,end_year_file)

model_raw_time,model_ref_time,model_datetime_time,model_data,lat_e,lon_e,lat_c,lon_c,grid_size,gridbox_count = modules.read_model_all(model_file,species,start_year,end_year)

n_levs = 47

#setup netcdf
root_grp_out = Dataset('model_sig_periods.nc', 'w')
root_grp_out.description = 'Amplitudes and Phases for key periods from Lomb-Scargle Periodogram analysis of Model in Vertical - Program written by Dene Bowdalo'
...
for j in range(len(obs_refs)):
    ...
    #get model cut for each site using lat/lon
    lat = obs_lats[j]
    lon = obs_lons[j]
    lat_n,lon_n = modules.obs_model_gridbox(lat_e,lon_e,lat,lon) 
    cut_data = model_data[:,:,lat_n,lon_n]

    for i in range(n_levs):
        linear_data.append(cut_data[:,i])  

    if __name__ == '__main__':
        pool = multiprocessing.Pool(processes=16)
        results = [pool.apply_async(run_LSP, (linear_data[x],lon,x)) for x in range(n_levs)]
