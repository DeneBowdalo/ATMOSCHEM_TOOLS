...
for ax in axes.flat:
    ...
    no2_box_edges = np.arange(0,no2_max[count]+0.001,0.5)
    co_box_edges = np.arange(0,co_max[count]+0.001,10) 
    
    fill_box_obs = np.empty((len(no2_box_edges)-1,len(co_box_edges)-1))
    fill_box_obs[:,:] = np.NaN
    
    fill_box_model = np.empty((len(no2_box_edges)-1,len(co_box_edges)-1))
    fill_box_model[:,:] = np.NaN
    ...
    for x in range(len(no2_box_edges)-1):
        for y in range(len(co_box_edges)-1):  
            ...
            test_obs = (lower_no2 <= obs_no2_cut_ave) & (upper_no2 > obs_no2_cut_ave) & (lower_co <= obs_co_cut_ave) & (upper_co > obs_co_cut_ave)
            test_model = (lower_no2 <= model_no2_cut_ave) & (upper_no2 > model_no2_cut_ave) & (lower_co <= model_co_cut_ave) & (upper_co > model_co_cut_ave)
            
            box_ave_o3_obs = np.nanmean(obs_o3_cut_ave[test_obs])
            box_ave_o3_model = np.nanmean(model_o3_cut_ave[test_model])
            
            fill_box_obs[x,y] = box_ave_o3_obs
            fill_box_model[x,y] = box_ave_o3_model
    ...
    extent = np.min(no2_box_edges), np.max(no2_box_edges), np.min(co_box_edges), np.max(co_box_edges)
    
    im1 = ax.imshow(np.transpose(fill_box_obs), cmap=plt.cm.Greys, interpolation='nearest', extent=extent,vmin = 0,vmax=70,aspect='auto',origin="lower")
    
    ax.hold(True)
    
    im2 = ax.imshow(np.transpose(fill_box_model), cmap=plt.cm.Reds, interpolation='nearest', extent=extent,vmin = 0,vmax=70,aspect='auto',alpha=0.6,origin="lower")
    #cb = ax.pcolor(xx,yy,np.transpose(fill_box_obs),vmin=0,vmax=70,cmap=plt.cm.Greys)
    #cb.cmap.set_under('white')
    #cb2 = ax.contour(xx,yy,np.transpose(fill_box_model),vmin=0,vmax=70,cmap=plt.cm.Reds)
    #cb2.cmap.set_under('white')
